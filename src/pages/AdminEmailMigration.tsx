...
const STORAGE_KEY = "pe.email-migration.checklist.v1";
...
function loadState(): ChecklistSection[] {
  ...
  try {
    ...
    // Merge: keep statuses/notes from saved, structure from INITIAL
      ...
      <div className="mx-auto max-w-5xl">
        <header className="mb-12 border-b border-border/40 pb-8">
          ...
          <p className="mt-4 max-w-2xl text-sm font-light text-muted-foreground/80 leading-relaxed">
            Tracking the cut-over from <span className="font-mono">peninsulaequine.org</span> to{" "}
            <span className="font-mono">peninsulaequine.systems</span>. Click a status pill to cycle Pending → Pass → Fail.
          </p>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-6 text-xs uppercase tracking-[0.3em]">
            ...
            <div className="ml-auto flex gap-2">
              <Button size="sm" variant="outline" onClick={runSmokeTest} disabled={running}>
          ...
          {sections.map((section) => (
            <section key={section.id}>
              <div className="mb-6">
                ...
                <p className="mt-1 text-sm font-light text-muted-foreground/70">{section.description}</p>
              </div>
                  ...
                  return (
                    <li key={item.id} className="py-4 flex items-start gap-6">
                      <button
                        ...
                        className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border ${meta.className} transition-colors`}
